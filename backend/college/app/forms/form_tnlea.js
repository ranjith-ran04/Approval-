...
async function form_tnlea(req, res) {
  ...
  if (result.length == 0) {
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="${allot_coll_code}.pdf"`
    );
    // res.send(data);
    ...
    const padding = 3;
    let currentY = doc.page.margins.top + 100;
    function TableHeader(yPosition) {
      if (yPosition < doc.page.margins.top + 100) {
        yPosition = doc.page.margins.top + 100;
      }
    ...
    function drawDataRow(row, serial, yPosition) {
      ...
      rowData.forEach((data, idx) => {
        doc
          .rect(x, yPosition - 4.5, columns[idx].width, dynamicrowheight)
          .stroke();
    ...
    footer("TNLEA", doc, allot_coll_code, freezed);
    doc.end();
  }
}
