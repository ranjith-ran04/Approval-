...
const collegeDetails = async (req, res) => {
  ...
  const [[result]] = await db.query(
    "SELECT * FROM college_info WHERE c_code = ?",
    [c_code]
  );
  ...
  res.setHeader(
    "Content-Disposition",
    'inline; filename="college_details.pdf"'
  );
  ...
  for (let index = 0; index < 22; index++) {
    ...
    const valueHeight = doc.heightOfString(result[key], {
      width: valWidth - 10,
    });
    ...
    const val = booleanFields.includes(key)
      ? value == 1
        ? "Yes"
        : "No"
      : value;
  ...
  doc
    .rect(startX + keyWidth + valWidth / 2, startY, valWidth / 2, rowheight)
    .stroke();
  ...
  doc
    .font(arialBold)
    .text("Girls Hostel", (valWidth - 10) / 2 + keyWidth + 60, startY + 7, {
      width: (valWidth - 10) / 2,
      align: "center",
    });
  ...
  hostelFiels.forEach((element) => {
    ...
    const boysVal = booleanFields.includes(boys)
      ? result[boys] == 1
        ? "Yes"
        : "No"
      : result[boys];
    const girlsVal = booleanFields.includes(girls)
      ? result[girls] == 1
        ? "Yes"
        : "No"
      : result[girls];
